...
function func_print_help() {
  ...
  usage:
	...
	--usb: usb device to install to, ie /dev/sdc
	--ncl: no cleanup, which means do not delete the modified copy of the iso contents
	

	if you are unsure;
	
	- ubuntu images can be downloaded from http://cdimage.ubuntu.com/release , ie:
...
if [ -z "$TARGET_USB_MEDIUM" ]; then
	...
	echo ""
	echo "do you want to check the above usb mediums and provide one with --usb option?"
	exit 127
fi

if [ ! -e "${TARGET_USB_MEDIUM}" ]; then
...
echo "apt-get -y update && apt-get install -y qemu qemu-kvm -bios ovmf"
...
echo "to use (u)efi, you can add the following bios option; -bios /usr/share/qemu/OVMF.fd"
